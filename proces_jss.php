...
switch($op1)
{
	case 'isp_add':
		...
		if($id>0)
		{
			...
			{
				$list = explode("&",$inpt);
				if(isset($_GET['ctrl'])) $isps = explode("|",@$list[1]);
				else $isps = explode("|",$list[0]);
				if($in>0)
				{
					$isps[] = $in;
					$isp = implode("|",$isps);
				...
				{
					$new_arr = array();
     				foreach($isps as $item)if($item!=abs($in))$new_arr[] = $item;
     				$isp = implode("|",$new_arr);
				}
				if(isset($_GET['ctrl'])) $isp = $list[0].'&'.$isp;
				else $isp = $isp.'&'.@$list[1];
			}elseif($in>0) {
				if(isset($_GET['ctrl'])) $isp ='&'.$in;
				else $isp = $in.'&';
			}
	...
	case 'list':
		...
		if($id>0)
		{
			...
			{
				if(!empty($vals[955])) $comm .= '<div><b>'.$vals[955].':</b> '.nl2br($vals[956]).'</div>';
				else $opis .= nl2br($vals[956]).'<br/>';
			}
	...
	case 'add':
		...
		if(!empty($_POST['auth']))$auth = iconv("UTF-8", "WINDOWS-1251", $_POST['auth']);
		if($id>0 && !empty($msg))
		{
			$data = array();
			if(!empty($auth))$data[955] = $auth;
			$data[956] = $msg;
			insertData($id,$data);
	...
	default:
		...
		function print_rub($pid=0,$i=0)
		{
			...
			while($row = mysql_fetch_array($res)){
				...
				if(empty($childs))
				{
					...
					if(count($data)==0)$opis = ' <span>&lt;без описания&gt;</span>';
				}
		...
		if($_USER['id']>0)
		{
            ...
            otherbase(5);
        }else $user = 'Администратор';
		$orderby=''; $limit=''; $features = array(916);$fvalues=false;$uslovia=array();
        $data = array();
		if(count($features)==0)
		{
		...
		uksort($data,"cmp");
		$dolz = '';
		foreach($data as $gid=>$vals)$dolz.='<option value="'.$gid.'">'.$vals[916].'</option>';
		$jss = <<<TXT
var user='$user';
